  ...
  return (
    <div className="glass-card rounded-2xl p-4">
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        >
          <div className="w-20 h-20 rounded-full bg-gradient-to-br from-purple-500 to-fuchsia-500 flex items-center justify-center shadow-lg">
        ...
        <div className="flex-1 space-y-1">
          <div className="flex justify-between text-xs">
            ...
            <span className="font-bold text-purple-700">{streak.current} day{streak.current !== 1 ? "s" : ""}</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-purple-400 font-medium">Longest ever</span>
            <span className="font-bold text-purple-700">{streak.longest} day{streak.longest !== 1 ? "s" : ""}</span>
          </div>
          {nextMilestone && (
            <div className="flex justify-between text-xs">
              <span className="text-purple-400 font-medium">Next milestone</span>
              <span className="font-bold text-fuchsia-500">{nextMilestone.days} days</span>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Achieved milestone messages */}
      {achieved.length > 0 && (
        <div className="mt-3 space-y-1.5">
          {achieved.slice(-3).reverse().map((m) => (
            <div key={m.days} className="text-xs text-purple-500 font-medium bg-purple-50/50 rounded-lg px-3 py-2">
              ⭐ {m.message}
            </div>
          ))}
        </div>
