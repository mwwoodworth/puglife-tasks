  ...
  return (
    <div className="glass-card rounded-2xl p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-bold text-purple-600 flex items-center gap-2"><span>🎯</span> Goal Progress</h3>
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="relative flex-shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className="text-lg font-black text-purple-700">{pct}%</span>
        ...
        <div className="flex-1 space-y-1.5">
          <div className="flex justify-between text-xs">
            ...
            <span className="font-bold text-purple-700">{progress?.totalLost.toFixed(1)} lbs</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-purple-400 font-medium">Current</span>
            <span className="font-bold text-purple-700">{progress?.currentWeight.toFixed(1)} lbs</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-purple-400 font-medium">Goal</span>
            <span className="font-bold text-fuchsia-600">{goal.goalWeight} lbs</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-purple-400 font-medium">Avg/Week</span>
            <span className="font-bold text-purple-700">{progress?.averagePerWeek.toFixed(1)} lbs</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-purple-400 font-medium">Trend</span>
            <span className={`font-bold ${progress?.trend === "losing" ? "text-green-500" : progress?.trend === "gaining" ? "text-red-400" : "text-purple-400"}`}>
              {progress?.trend === "losing" ? "📉 Losing" : progress?.trend === "gaining" ? "📈 Gaining" : "➡️ Steady"}
            </span>
          </div>
        </div>
      </div>
