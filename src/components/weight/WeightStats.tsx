    ...
    <div className="space-y-3">
      {bmi && bmiCat && (
        <div className="glass-card rounded-2xl p-4">
          <h3 className="text-sm font-bold text-purple-200 mb-2 flex items-center gap-2"><span>🧮</span> BMI</h3>
      ...
      {sorted.length > 0 && (
        <div className="glass-card rounded-2xl p-4">
          <h3 className="text-sm font-bold text-purple-200 mb-3 flex items-center gap-2"><span>📋</span> Recent Entries</h3>
              ...
              {sorted.slice(0, 15).map((entry, i) => {
                ...
                return (
                  <motion.div
                    ...
                    exit={{ opacity: 0, x: 10 }}
                    className="flex items-center justify-between py-2 border-b border-purple-500/15 last:border-0"
                  >
                    <div className="flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      {entry.note && <span className="text-[10px] text-purple-500 max-w-[80px] truncate">{entry.note}</span>}
                      <button onClick={() => onDeleteEntry(entry.id)} className="text-xs text-purple-500 active:text-red-400 p-1">✕</button>
                    </div>
                  </motion.div>
                );
              })}
